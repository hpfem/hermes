...
static void load_funcs(void)
{
  ...
  dbfd_init = (void(*)())dlsym(handle, "bfd_init");
  dbfd_scan_vma = (bfd_vma(*)(const char *, const char **, int))dlsym(handle, "bfd_scan_vma");
  dbfd_openr = (bfd*(*)(const char *, const char *))dlsym(handle, "bfd_openr");
  dbfd_check_format = (bfd_boolean(*)(bfd *, bfd_format))dlsym(handle, "bfd_check_format");
  dbfd_check_format_matches = (bfd_boolean(*)(bfd *, bfd_format, char ***))dlsym(handle, "bfd_check_format_matches");
  dbfd_close = (bfd_boolean(*)(bfd *))dlsym(handle, "bfd_close");
  dbfd_map_over_sections = (bfd_boolean(*)(bfd *, void(*func)(bfd *, asection *, void *obj), void *)) dlsym(handle, "bfd_map_over_sections");
}
...
#define OPTION_DEMANGLER      (150)
  ...
  static void find_address_in_section(bfd *abfd, asection *section, void *data __attribute__((__unused__)))
  {
    ...
    found = bfd_find_nearest_line(abfd, section, syms, pc - vma, (const char**)&filename, &functionname, &line);
  }
  ...
  static char** translate_addresses_buf(bfd * abfd, bfd_vma *addr, int naddr)
  {
    ...
    for (int stateI = Count; stateI <= Print; stateI++) {
      if (stateI == (int)Print) {
...
#define MAX_DEPTH 16
    ...
    size_t size, void *data)
  {
    ...
    for (n = info->dlpi_phnum; --n >= 0; phdr++) {
      if (phdr->p_type == PT_LOAD) {
        ...
        if ((long)match->address >= vaddr && (long)match->address < vaddr + phdr->p_memsz) {
          /* we found a match */
  ...
  char **backtrace_symbols(void *const *buffer, int size)
  {
    ...
    for (x = stack_depth, y = 0; x >= 0; x--, y++){
      ...
      addr = (long)buffer[x] - (long)match.base;

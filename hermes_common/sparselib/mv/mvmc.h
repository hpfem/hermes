...
{                                                                      
    ...
    inline const COMPLEX& operator()( int,  int) const; 
    ...
    MV_ColMat_COMPLEX&        newsize( int,  int);
    ...
    friend std::ostream& operator<<(std::ostream &s, const MV_ColMat_COMPLEX &A);

};                                                                     

inline COMPLEX& MV_ColMat_COMPLEX::operator()( int i,  int j)
{
#ifdef MV_MATRIX_BOUNDS_CHECK
    assert(0<=i && i<dim(0));
    assert(0<=j && j<dim(1));
...
inline MV_ColMat_COMPLEX::MV_ColMat_COMPLEX(COMPLEX* d,  int m, 
...
inline MV_ColMat_COMPLEX::MV_ColMat_COMPLEX(COMPLEX* d,  int m,  int n,

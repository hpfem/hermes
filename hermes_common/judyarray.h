...
#ifndef _HERMES_JUDYARRAY_H_
#define _HERMES_JUDYARRAY_H_
...
template<class TYPE>
bool JudyArray<TYPE>::set(Word_t idx, TYPE item) {
	void *pval;
	JLG(pval, judy, idx);
	if (pval == NULL) {
		JLI(pval, judy, idx);
...
template<class TYPE>
Word_t JudyArray<TYPE>::add(TYPE item) {
	int rc;
	Word_t idx = last();
...
template<class TYPE>
bool JudyArray<TYPE>::exists(Word_t idx) const {
	void *pval;
	JLG(pval, judy, idx);
	return pval != NULL;
}
...
template<class TYPE>
TYPE JudyArray<TYPE>::get(Word_t idx) const {
	void *pval;
	JLG(pval, judy, idx);
	assert(pval != NULL);
	return **((TYPE **) pval);
...
template<class TYPE>
TYPE JudyArray<TYPE>::operator[](Word_t idx) const {
	return get(idx);
}

template<class TYPE>
TYPE &JudyArray<TYPE>::operator[](Word_t idx) {
	void *pval;
	JLG(pval, judy, idx);
	if (pval == NULL) {
		JLI(pval, judy, idx);
...
template<class TYPE>
void JudyArray<TYPE>::free_item(Word_t idx) {
	void *pval;
	JLG(pval, judy, idx);
	if (pval != NULL) {
		TYPE *n = *((TYPE **) pval);
...
template<class TYPE>
void JudyArray<TYPE>::remove(Word_t idx) {
	free_item(idx);
	int rc;
	JLD(rc, judy, idx);
}
...
template<class TYPE>
Word_t JudyArray<TYPE>::count(Word_t index1/* = 0*/, Word_t index2/* = (Word_t) -1*/) const {
	Word_t count;
...
template<class TYPE>
Word_t JudyArray<TYPE>::by_count(Word_t nth) const {
	void *pval;
	Word_t index;
...
template<class TYPE>
Word_t JudyArray<TYPE>::mem_used() const {
	Word_t memused;
	JLMU(memused, judy);
	return memused;
}
...
template<class TYPE>
void JudyArray<TYPE>::remove_all() {
...
template<class TYPE>
Word_t JudyArray<TYPE>::first(Word_t idx/* = 0*/) const {
	void *pval;
	JLF(pval, judy, idx);
...
template<class TYPE>
Word_t JudyArray<TYPE>::next(Word_t idx/* = 0*/) const {
	void *pval;
	JLN(pval, judy, idx);
	return pval ? idx : INVALID_IDX;
}
...
template<class TYPE>
Word_t JudyArray<TYPE>::last(Word_t idx/* = (Word_t) -1*/) const {
	void *pval;
	JLL(pval, judy, idx);
	return pval ? idx : INVALID_IDX;
}
...
template<class TYPE>
Word_t JudyArray<TYPE>::prev(Word_t idx/* = (Word_t) -1*/) const {
	void *pval;
	JLP(pval, judy, idx);
	return pval ? idx : INVALID_IDX;
}

...
int main(int argc, char* argv[])
{
  ...
  wf_left.add_matrix_form(bilinear_form_left, bilinear_form_ord, HERMES_SYM, HERMES_ANY,Hermes::vector<MeshFunction*>(&pot_exact, &wfun_exact,&coul_pot ));
  ...
  WeakForm wf_coulomb;
  wf_coulomb.add_matrix_form(bilinear_form_coul_pot,bilinear_form_ord1,HERMES_SYM, HERMES_ANY,Hermes::vector<MeshFunction*>(&wfun_exact,&coul_pot ));
  ...
  while (!DONE){
    ...
    for (int ieig = 0; ieig < neig; ieig++) {
      ...
      info("eigenvector %d : norm=%25.16f \n eigenvalue=%25.16f \n coulomb_energy=%25.16f ",ieig,pow(norm2,0.5),eival[ieig],coulomb_energy);
      wf.add_vector_form(linear_form_poisson,linear_form_poisson_ord,HERMES_ANY,Hermes::vector<MeshFunction*>(&sln, &wfun_exact )); 

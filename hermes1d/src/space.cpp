             ...
             int p_init, int n_eq, int n_sln, bool print_banner)
{
  ...
  if(left_boundary_conditions != Hermes::vector<std::pair<int, double> *>())
             ...
             int n_eq, int n_sln, bool print_banner)
{
  ...
  // NO BOUNDARY CONDITIONS!!! DO NOT FORGET TO ASSIGN DOFS AFTER SETTING B.C. EXTERNALLY!
  this->assign_dofs();
}
...
int Space::assign_dofs()
{
  ...
  if(DEBUG_ELEM_DOF) {
    ...
    info("Elements = %d", this->n_base_elem);
...
{
  info("Element (%g, %g): refinement candidates:", this->x1, this->x2);
  for (int i=0; i < num_cand; i++) { 
    info("%d %d %d", cand_list[i][0], cand_list[i][1], cand_list[i][2]);
  }
}
		          ...
		          int subdivision)
{
  ...
  fclose(f);
  info("Error function written to %s.", final_filename);
}

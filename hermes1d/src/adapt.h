...
double H1D_API calc_elem_est_error_squared_p(int norm, Element *e, Element *e_ref,
        int sln=0);
...
// reference solution.
// FIXME: to be moved to the Element class
double H1D_API calc_elem_est_error_squared_hp(int norm, Element *e, 
...
// error is returned.
double H1D_API calc_error_estimate(int norm, Mesh* mesh, Mesh* mesh_ref, 
			   double *err_array, int sln=0);

// Calculates L2 or H1 norm of the difference between the coarse
// and reference solutions in all active elements of 'mesh'. Total
// error is returned.
double H1D_API calc_error_estimate(int norm, Mesh* mesh, 
                           ElemPtr2* ref_element_pairs);

// Can be used for both the coarse and reference solutions
double H1D_API calc_solution_norm(int norm, Mesh* mesh);
double H1D_API calc_solution_norm(int norm, Mesh* mesh, ElemPtr2* ref_element_pairs);

// Sort err_array[] and returning array of sorted element indices
void H1D_API sort_element_errors(int n, double *err_array, int *id_array); 
...
void H1D_API check_cand_coarse_hp_fine_hp(Element *e, Element *e_ref_left, Element *e_ref_right, 
...
// number of dof added by this candidate.
void H1D_API check_cand_coarse_hp_fine_p(Element *e, Element *e_ref,
                                 int p_left, int p_right,
                                 double &err, int &dof);
...
// number of dof added by this candidate.
void H1D_API check_cand_coarse_p_fine_hp(Element *e, Element *e_ref_left, Element *e_ref_right, 
...
// polynomials of degree 'p' on 'e'. Returned is projection error and
// number of dof added by this candidate.
void H1D_API check_cand_coarse_p_fine_p(Element *e, Element *e_ref, int p,
                                double &err, int &dof);

// Error wrt. exact solution (if provided) on element 'e' 
double H1D_API calc_elem_exact_error_squared(int norm, exact_sol_type exact_sol,
                                     Element *e, int order);

// Error wrt. exact solution (if provided) on the entire interval (A, B) 
double H1D_API calc_error_exact(int norm, Mesh *mesh, 
                        exact_sol_type exact_sol); 

// Calculates L2 or H1 norm of function exact_sol in interval (A, B)
double H1D_API calc_solution_norm(int norm, exact_sol_type exact_sol, 
                           int n_eq, 
...
int H1D_API select_hp_refinement(Element *e, Element *e_ref, Element * e_ref2,

            ...
            bool verbose)
{
  ...
  while (1) {
    ...
    for(int i=0; i<n_dof; i++) res_norm_squared += rhs->get(i)*rhs->get(i);
    ...
    if(res_norm_squared < newton_tol*newton_tol) break;
    ...
    if (solved) 
    {
      double* solution_vector = new double[n_dof];
      solution_vector = solver->get_solution();
      for(int i=0; i<n_dof; i++) y[i] += solution_vector[i];

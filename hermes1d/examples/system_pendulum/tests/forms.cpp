                ...
                void *user_data)
{
  ...
  for(int i = 0; i<num; i++) {
    val += k*k * cos(u_prev[si][0][i])*u[i]*v[i]*weights[i];
  }
  return val;
};
                ...
                double *v, double *dvdx, void *user_data)
{
  ...
  int si = 0;      // solution index (only 0 is relevant for this example)
  for(int i = 0; i<num; i++) {
    val += (du_prevdx[si][0][i]*v[i] - u_prev[si][1][i]*v[i])*weights[i];
  }
  return val;
};
                ...
                double *v, double *dvdx, void *user_data)
{
  ...
  int si = 0;      // solution index (only 0 is relevant for this example)
  for(int i = 0; i<num; i++) {
    val += (k*k*sin(u_prev[si][0][i])*v[i] + du_prevdx[si][1][i]*v[i])*weights[i];
  }
  return val;
};

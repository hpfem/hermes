                         ...
                         Geom<double> *e, ExtData<double> *ext)
{
  double result = 0;
  for (int i = 0; i < n; i++) {
    result += -wt[i] * (u->dx[i] * v->dx[i] + u->dy[i] * v->dy[i]) * lambda(e->x[i], e->y[i]);
  }
                         ...
                         Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    result += -wt[i] * ( sln_dx_i * v->dx[i] +  sln_dy_i * v->dy[i]) * lambda(e->x[i], e->y[i]);	       
  }
                         ...
                         Geom<double> *e, ExtData<double> *ext)
{
  double result = 0;
  for (int i = 0; i < n; i++) {
    result += wt[i] * u->val[i] * v->val[i] * lambda(e->x[i], e->y[i]);
  }
                         ...
                         Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    result += wt[i] * (TEMP_EXT_TOP - sln_val_i) * v->val[i] * lambda(e->x[i], e->y[i]);		       
  }
                         ...
                         Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) {
    double sln_val_i = sln_prev_time->val[i] + K_sln->val[i];
    result += wt[i] * (T_fire(e->x[i], stage_time) - sln_val_i) * v->val[i] * lambda(e->x[i], e->y[i]);		       
  }

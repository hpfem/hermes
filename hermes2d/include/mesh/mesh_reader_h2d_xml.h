...
namespace Hermes
{
  namespace Hermes2D
  {
    ...
    protected:
      ...
      bool load(std::auto_ptr<XMLSubdomains::domain> & parsed_xml_domain, Mesh *mesh, std::map<unsigned int, unsigned int>& vertex_is, int* element_is, std::map<unsigned int, unsigned int>& edge_is);

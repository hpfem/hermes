...
namespace Hermes
{
  namespace Hermes2D
  {
    ...
    private:
      ...
      Func<Hermes::Ord>** init_ext_orders(MeshFunctionSharedPtrVector<Scalar>& ext, Hermes::vector<UExtFunctionSharedPtr<Scalar> >& u_ext_fns, Func<Hermes::Ord>** u_ext_func);
      /// \ingroup Helper methods inside {calc_order_*, assemble_*}
      ...
      int calc_order_dg_matrix_form(const SpaceSharedPtrVector<Scalar> spaces, Traverse::State* state, MatrixFormDG<Scalar>* mfDG, RefMap** current_refmaps, Solution<Scalar>** current_u_ext, bool neighbor_supp_u, bool neighbor_supp_v, NeighborSearch<Scalar>** neighbor_searches);

      /// Calculates integration order for DG vector forms.
      int calc_order_dg_vector_form(const SpaceSharedPtrVector<Scalar> spaces, Traverse::State* state, VectorFormDG<Scalar>* vfDG, RefMap** current_refmaps, Solution<Scalar>** current_u_ext, bool neighbor_supp_v, NeighborSearch<Scalar>** neighbor_searches);
      ...
      DiscontinuousFunc<Hermes::Ord>** init_ext_fns_ord(MeshFunctionSharedPtrVector<Scalar> &ext,

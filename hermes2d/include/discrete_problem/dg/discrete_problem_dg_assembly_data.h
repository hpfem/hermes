...
namespace Hermes
{
  namespace Hermes2D
  {
    ...
    class HERMES_API DiscreteProblemDGAssemblyData : 
      public DiscreteProblemDGAssemblyDataInterface<Scalar>, 
    ...
    public:
      ...
      DiscreteProblemDGAssemblyData(WeakForm<Scalar>* wf, Hermes::vector<SpaceSharedPtr<Scalar> > spaces);

      /// Constructor for one equation.
      DiscreteProblemDGAssemblyData(WeakForm<Scalar>* wf, SpaceSharedPtr<Scalar> space);

      /// State querying helpers.
      virtual bool isOkay() const;
      virtual inline std::string getClassName() const { return "DiscreteProblemDGAssemblyData"; }
      ...
      DiscreteProblemDGAssemblyData();

      /// Destuctor.
      virtual ~DiscreteProblemDGAssemblyData();
    ...
    protected:
      ...
      typename DiscreteProblemDGAssemblyDataCache<Scalar>::CacheRecord* get_state_cache(Traverse::State* state, PrecalcShapeset** current_pss, RefMap** current_refmaps, Solution<Scalar>** current_u_ext, AsmList<Scalar>** current_als, AsmList<Scalar>*** current_alsSurface, WeakForm<Scalar>* current_wf, int& order);

      /// Assemble one state.
      void assemble_one_state(typename DiscreteProblemDGAssemblyDataCache<Scalar>::CacheRecord* cache_record, RefMap** current_refmaps, Solution<Scalar>** current_u_ext, AsmList<Scalar>** current_als, Traverse::State* current_state, WeakForm<Scalar>* current_wf);
      ...
      /// The cache.
      DiscreteProblemDGAssemblyDataCache<Scalar> cache;

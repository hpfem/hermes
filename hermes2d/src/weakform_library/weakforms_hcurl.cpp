  ...
  namespace Hermes2D
  {
    ...
    namespace WeakFormsHcurl
    {
        ...
        : MatrixFormVol<Scalar>(i, j), const_coeff(const_coeff), function_coeff(f_coeff), gt(gt)
      {
        ...
        // If f_coeff is NULL, initialize it to be constant 1.0.
        if(f_coeff == NULL) this->function_coeff = new Hermes2DFunction<Scalar>(1.0);
        ...
        function_coeff0(f_coeff0), function_coeff1(f_coeff1), gt(gt)
      {
        ...
        // If f_coeff0 is NULL, initialize it to be constant 1.0.
        if(f_coeff0 == NULL) this->function_coeff0 = new Hermes2DFunction<Scalar>(1.0);
        else throw Hermes::Exceptions::Exception("Nonconstant coefficients in Hcurl forms not implemented yet.");
        if(f_coeff1 == NULL) this->function_coeff1 = new Hermes2DFunction<Scalar>(1.0);
        else throw Hermes::Exceptions::Exception("Nonconstant coefficients in Hcurl forms not implemented yet.");
      }
      ...
      {
        if(function_coeff0 == NULL) delete function_coeff0;
        if(function_coeff1 == NULL) delete function_coeff1;
      };
      ...
      {
        if(function_coeff == NULL) delete function_coeff;
      };
      ...
      {
        if(spline_coeff == NULL) delete spline_coeff;
      };

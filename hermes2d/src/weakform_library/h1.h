...
namespace WeakFormsH1 {
    ...
    */
    ...
    /* Default volumetric matrix form \int_{area} const_coeff * spline_coeff'(u_ext[j]) u \nabla u_ext[i] \cdot \nabla v
       + const_coeff * spline_coeff(u_ext[j]) * \nabla u \cdot \nabla v d\bfx
       const_coeff... constant number
    ...
    */
    ...
    public:
      DefaultJacobianDiffusion(int i, int j, std::string area = HERMES_ANY, scalar const_coeff = 1.0,
                               ...
                               SymFlag sym = HERMES_NONSYM, GeomType gt = HERMES_PLANAR)
        : WeakForm::MatrixFormVol(i, j, area, sym), idx_j(j), const_coeff(const_coeff), spline_coeff(c_spline), gt(gt)
      {
      ...
      DefaultJacobianDiffusion(int i, int j, Hermes::vector<std::string> areas, scalar const_coeff = 1.0,
                               CubicSpline* c_spline = HERMES_DEFAULT_SPLINE,
                               SymFlag sym = HERMES_NONSYM, GeomType gt = HERMES_PLANAR)
        : WeakForm::MatrixFormVol(i, j, areas, sym), idx_j(j), const_coeff(const_coeff), spline_coeff(c_spline), gt(gt)
      {
        // If spline is HERMES_DEFAULT_SPLINE, initialize it to be constant 1.0.
        if (c_spline == HERMES_DEFAULT_SPLINE) this->spline_coeff = new CubicSpline(1.0);
      }
    ...
    /* Default volumetric matrix form
       \int_{area} spline_coeff1`(u_ext[j]) * u * u_ext[j]->dx * v
       + spline_coeff1(u_ext[j]) * u->dx * v
       + spline_coeff2`(u_ext[j]) * u * u_ext[j]->dy * v
       + spline_coeff2(u_ext[j]) * u->dy * v d\bfx.
    ...
    */
    ...
    /* Default volumetric vector form \int_{area} const_coeff * function_coeff(x, y) * u_ext[i] * v d\bfx
       const_coeff... constant number
       function_coeff... (generally nonconstant) function of x, y
    */
    ...
    public:
      DefaultResidualVol(int i, std::string area = HERMES_ANY, scalar const_coeff = 1.0,
                           DefaultFunction* f_coeff = HERMES_DEFAULT_FUNCTION,
                           GeomType gt = HERMES_PLANAR)
             : WeakForm::VectorFormVol(i, area), idx_i(i), const_coeff(const_coeff), function_coeff(f_coeff), gt(gt)
      {
        // If f_coeff is HERMES_DEFAULT_FUNCTION, initialize it to be constant 1.0.
        if (f_coeff == HERMES_DEFAULT_FUNCTION) this->function_coeff = new DefaultFunction(1.0);
      }
      DefaultResidualVol(int i, Hermes::vector<std::string> areas, scalar const_coeff = 1.0,
                           DefaultFunction* f_coeff = HERMES_DEFAULT_FUNCTION,
                           GeomType gt = HERMES_PLANAR)
             : WeakForm::VectorFormVol(i, areas), idx_i(i), const_coeff(const_coeff), function_coeff(f_coeff), gt(gt)
      {
        // If f_coeff is HERMES_DEFAULT_FUNCTION, initialize it to be constant 1.0.
        if (f_coeff == HERMES_DEFAULT_FUNCTION) this->function_coeff = new DefaultFunction(1.0);
      }
    ...
    /* Default volumetric vector form \int_{area} const_coeff * spline_coeff(u_ext[i]) *
       \nabla u_ext[i] \cdot \nabla v d\bfx
       const_coeff... constant number
    ...
    */
    ...
    public:
      DefaultResidualDiffusion(int i, std::string area = HERMES_ANY, scalar const_coeff = 1.0,
                               CubicSpline* c_spline = HERMES_DEFAULT_SPLINE,
                               GeomType gt = HERMES_PLANAR)
        : WeakForm::VectorFormVol(i, area), idx_i(i), const_coeff(const_coeff), spline_coeff(c_spline), gt(gt)
      {
        // If spline is HERMES_DEFAULT_SPLINE, initialize it to be constant 1.0.
        if (c_spline == HERMES_DEFAULT_SPLINE) this->spline_coeff = new CubicSpline(1.0);
      };
    ...
    /* Default volumetric vector form \int_{area} spline_coeff1(u_ext[i]) * u->dx * v->val
       + spline_coeff2(u_ext[i]) * u->dy * v->val d\bfx
       spline_coeff1, spline_coeff2... non-constant parameters given by cubic splines
    */
    ...
    */
    ...
    /* Default surface matrix form \int_{area} const_coeff * spline_coeff'(u_ext[j]) * u_ext[j] * u * v
       + const_coeff * spline_coeff(u_ext[j]) * u * v dS
    ...
    */
    ...
    public:
      DefaultJacobianFormSurf(int i, int j, std::string area = HERMES_ANY, scalar const_coeff = 1.0,
                              CubicSpline* c_spline = HERMES_DEFAULT_SPLINE,
                              GeomType gt = HERMES_PLANAR)
        : WeakForm::MatrixFormSurf(i, j, area), idx_j(j), const_coeff(const_coeff), spline_coeff(c_spline), gt(gt)
      {
        // If spline is HERMES_DEFAULT_SPLINE, initialize it to be constant 1.0.
        if (c_spline == HERMES_DEFAULT_SPLINE) this->spline_coeff = new CubicSpline(1.0);
      }
      DefaultJacobianFormSurf(int i, int j, Hermes::vector<std::string> areas, scalar const_coeff = 1.0,
                              CubicSpline* c_spline = HERMES_DEFAULT_SPLINE,
                              GeomType gt = HERMES_PLANAR)
        : WeakForm::MatrixFormSurf(i, j, areas), idx_j(j), const_coeff(const_coeff), spline_coeff(spline_coeff), gt(gt)
      {
        // If spline is HERMES_DEFAULT_SPLINE, initialize it to be constant 1.0.
        if (c_spline == HERMES_DEFAULT_SPLINE) this->spline_coeff = new CubicSpline(1.0);
      }
  ...
  return const_coeff * result;
    ...
    /* Default surface vector form \int_{area} const_coeff * function_coeff(x, y) * u_ext[i] v dS
       const_coeff... constant number
       function_coeff... (generally nonconstant) function of x, y
    */
    ...
    public:
      DefaultResidualSurf(int i, std::string area = HERMES_ANY, scalar const_coeff = 1.0,
                          DefaultFunction* f_coeff = HERMES_DEFAULT_FUNCTION,
                          GeomType gt = HERMES_PLANAR)
        : WeakForm::VectorFormSurf(i, area), idx_i(i), const_coeff(const_coeff), function_coeff(f_coeff), gt(gt)
      {
        // If f_coeff is HERMES_DEFAULT_FUNCTION, initialize it to be constant 1.0.
        if (f_coeff == HERMES_DEFAULT_FUNCTION) this->function_coeff = new DefaultFunction(1.0);
      }
      DefaultResidualSurf(int i, Hermes::vector<std::string> areas, scalar const_coeff = 1.0,
                          DefaultFunction* f_coeff = HERMES_DEFAULT_FUNCTION,
                          GeomType gt = HERMES_PLANAR)
        : WeakForm::VectorFormSurf(i, areas), idx_i(i), const_coeff(const_coeff), function_coeff(f_coeff), gt(gt)
      {
        // If f_coeff is HERMES_DEFAULT_FUNCTION, initialize it to be constant 1.0.
        if (f_coeff == HERMES_DEFAULT_FUNCTION) this->function_coeff = new DefaultFunction(1.0);
      }

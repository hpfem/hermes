...
namespace Hermes
{
  namespace Views
  {
    ...
    static std::map<int, Hermes::Views::View*> view_instances; ///< Instances of views.

    struct ViewParams { ///< Parameters used to initialize view
      Hermes::Views::View* view;
      int x, y;
      int width, height;
      const char* title;
      ViewParams(Hermes::Views::View* view, int x, int y, int width, int height, const char* title) : view(view), x(x), y(y), width(width), height(height), title(title) {};
    };
    ...
    static int set_view_title_in_thread(void* title_pars_ptr)
    {
      ...
      std::map<int, Hermes::Views::View*>::iterator found_view = view_instances.find(title_params.view_id);
    ...
    int remove_view_in_thread(void* remove_params_ptr)
    {
      ...
      std::map<int, Hermes::Views::View*>::iterator found_view = view_instances.find(params.view_id);
      if (found_view == view_instances.end()) 
      {
    ...
    static int refresh_view_in_thread(void* view_id_ptr) 
    {
      ...
      std::map<int, Hermes::Views::View*>::iterator found_view = view_instances.find(view_id);
...
#define STUB_GET_VIEW() Hermes::Views::View* wnd = (Hermes::Views::View*)glutGetWindowData() /* retrieves view for the current GLUT callback */
#define STUB_CALL(__call) STUB_GET_VIEW(); if (wnd != NULL) __call; /* calls a method of a view for the current GLUT callbakc */
    ...
    int add_view(Hermes::Views::View* view, int x, int y, int width, int height, const char* title) 
    {
    ...
    void force_view_thread_shutdown() 
    {
      ...
      //destroy all views
      std::map<int, Hermes::Views::View*>::const_iterator iter = view_instances.begin();

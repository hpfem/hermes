  ...
  namespace Hermes2D
  {
    ...
    void HdivSpace<Scalar>::init(Shapeset* shapeset, int p_init, bool assign_dofs_init)
    {
      ...
      if (assign_dofs_init)
      {
        ...
        if (p_init < 0)
          throw Hermes::Exceptions::Exception("P_INIT must be >= 0 in an Hdiv space.");
        else
    ...
    template<typename Scalar>
    void HdivSpace<Scalar>::copy(SpaceSharedPtr<Scalar> space, MeshSharedPtr new_mesh)
    {
    ...
    template<typename Scalar>
    void HdivSpace<Scalar>::set_shapeset(Shapeset *shapeset, bool clone)
    {
    ...
    void HdivSpace<Scalar>::get_bubble_assembly_list(Element* e, AsmList<Scalar>* al) const
    {
      ...
      short* indices = this->shapeset->get_bubble_indices(ed->order, e->get_mode());
    ...
    template<typename Scalar>
    void HdivSpace<Scalar>::update_constrained_nodes(Element* e, typename Space<Scalar>::EdgeInfo* ei0, typename Space<Scalar>::EdgeInfo* ei1, typename Space<Scalar>::EdgeInfo* ei2, typename Space<Scalar>::EdgeInfo* ei3)
    {
      int j;

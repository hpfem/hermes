...
DiscreteProblem::DiscreteProblem(WeakForm* wf_, Hermes::vector<Space*> sp)
{
  this->init(wf_);
  this->init_spaces(sp);
}
...
void DiscreteProblem::free_spaces()
{
  ...
  if (this->spaces != Hermes::vector<Space *>())
  {
    for (int i = 0; i < this->wf->neq; i++) {
...
void DiscreteProblem::init_spaces(Hermes::vector<Space*> spaces)
{
  int n = spaces.size();
  if (n != this->wf->neq)
...
void DiscreteProblem::init_space(Space* s)
{
  ...
  this->init_spaces(Hermes::vector<Space*>(s));
}

// Obsolete. Should be removed after FeProblem is removed.
void DiscreteProblem::set_spaces(Hermes::vector<Space*>spaces)
{
  this->init_spaces(spaces);
}

void DiscreteProblem::set_pss(Hermes::vector<PrecalcShapeset*> pss)
{
...
{
  this->set_pss(Hermes::vector<PrecalcShapeset*>(pss));
}
                               ...
                               Vector* rhs_ext, bool rhsonly, bool is_complex)
{
  ...
  // If init_vec != NULL, convert it to a Hermes::vector of solutions u_ext.
...
scalar DiscreteProblem::eval_form(WeakForm::MatrixFormVol *mfv, Hermes::vector<Solution *> sln, 
...
scalar DiscreteProblem::eval_form(WeakForm::VectorFormVol *vfv, Hermes::vector<Solution *> sln, PrecalcShapeset *fv, RefMap *rv)
{
  // Determine the integration order.
...
scalar DiscreteProblem::eval_form(WeakForm::MatrixFormSurf *mfs, Hermes::vector<Solution *> sln, 
...
scalar DiscreteProblem::eval_form(WeakForm::VectorFormSurf *vfs, Hermes::vector<Solution *> sln, 
...
int DiscreteProblem::assign_dofs()
{
  ...
  if (this->spaces == Hermes::vector<Space *>()) error("this->spaces is empty in DiscreteProblem::assign_dofs().");
  int ndof = 0;
  for (int i = 0; i < this->wf->neq; i++) {
...
void project_internal(Hermes::vector<Space *> spaces, WeakForm *wf, 
                      Hermes::vector<Solution*> target_slns, Vector* target_vec, bool is_complex)
{
  int n = spaces.size();
  ...
  if (target_slns != Hermes::vector<Solution *>()) {
...
void project_global(Hermes::vector<Space *> spaces, Hermes::vector<int> proj_norms, Hermes::vector<MeshFunction*> source_meshfns, 
                    Hermes::vector<Solution*> target_slns, Vector* target_vec, bool is_complex)
{
  int n = spaces.size();  
  ...
  for (int i = 0; i < n; i++) {
    int norm;
    if (proj_norms == Hermes::vector<int>()) norm = 1;
...
void project_global(Hermes::vector<Space *> spaces, matrix_forms_vector_t proj_biforms,
                    vector_forms_vector_t proj_liforms, Hermes::vector<MeshFunction*> source_meshfns,
                    Hermes::vector<Solution*> target_slns, Vector* target_vec, bool is_complex)
{
  int n = spaces.size();
  matrix_forms_vector_t::size_type n_biforms = proj_biforms.size();
  if (n_biforms == 0)
    error("Please use the simpler version of project_global with the argument Hermes::vector<int> proj_norms if you do not provide your own projection norm.");
...
int get_num_dofs(Hermes::vector<Space *> spaces)
{
  int ndof = 0;
...
int DiscreteProblem::get_num_dofs()
{
  ...
  if (this->spaces == Hermes::vector<Space *>()) error("this->spaces is empty in DiscreteProblem::get_num_dofs().");
...
bool solve_newton(Hermes::vector<Space *> spaces, WeakForm* wf, Vector* coeff_vec, 
...
HERMES_API bool solve_newton_adapt(Hermes::vector<Space *> spaces, WeakForm* wf, Vector *coeff_vec, 
                        MatrixSolverType matrix_solver, Hermes::vector<int>proj_norms, 
                        Hermes::vector<Solution *> target_slns, Hermes::vector<Solution *> target_ref_slns, 
                        Hermes::vector<WinGeom *> sln_win_geom, Hermes::vector<WinGeom *> mesh_win_geom, 
                        Hermes::vector<RefinementSelectors::Selector *> selectors, AdaptivityParamType* apt,
                        double newton_tol_coarse, double newton_tol_fine, int newton_max_iter, 
                        bool verbose, Hermes::vector<ExactSolution *> exact_slns, 
                        bool is_complex)
{
  // Time measurement.
  TimePeriod cpu_time;
  ...
  for (int i = 0; i < num_comps; i++) {
    ...
    if (sln_win_geom != Hermes::vector<WinGeom *>() && sln_win_geom[i] != NULL) {
    ...
    if (mesh_win_geom != Hermes::vector<WinGeom *>() && mesh_win_geom[i] != NULL) {
  ...
  Hermes::vector<Solution *> slns = Hermes::vector<Solution *>();
  Hermes::vector<Solution *> ref_slns = Hermes::vector<Solution *>();
  for (int i = 0; i < num_comps; i++) {
  ...
  Hermes::vector<MeshFunction*> slns_mf = Hermes::vector<MeshFunction*>();
  Hermes::vector<MeshFunction*> ref_slns_mf = Hermes::vector<MeshFunction*>();
  for (int i = 0; i < num_comps; i++) {
  ...
  do {
    ...
    Hermes::vector<Space *> ref_spaces = Hermes::vector<Space *>();
    Hermes::vector<Mesh *> ref_meshes = Hermes::vector<Mesh *>();
    for (int i = 0; i < num_comps; i++) {
    ...
    if (as == 1) {
      ...
      project_global(ref_spaces, proj_norms, slns_mf, Hermes::vector<Solution*>(), coeff_vec, is_complex);
    }
    else {
      info("Projecting previous fine mesh solution to obtain initial vector on new fine mesh.");
      project_global(ref_spaces, proj_norms, ref_slns_mf, Hermes::vector<Solution*>(), coeff_vec, is_complex);
    }
  ...
  if (target_slns != Hermes::vector<Solution *>()) {
    if (slns.size() != target_slns.size()) error("Mismatched solution Hermes::vector length in solve_newton_adapt().");
    for (int i = 0; i < num_comps; i++) {
  ...
  if (target_ref_slns != Hermes::vector<Solution *>()) {
    if (ref_slns.size() != target_ref_slns.size()) 
      error("Mismatched reference solution Hermes::vector length in solve_newton_adapt().");
    for (int i = 0; i < num_comps; i++) {

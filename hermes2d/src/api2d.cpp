  ...
  namespace Hermes2D
  {
    ...
    Api2D::Api2D()
    {
      ...
      if (ss.str().at(ss.str().length() - 1) == '\\' || ss.str().at(ss.str().length() - 1) == '/')
        this->text_parameters.insert(std::pair<Hermes2DApiParam, Parameter<std::string>*>(Hermes::Hermes2D::precalculatedFormsDirPath, new Parameter<std::string>(std::string(H2D_PRECALCULATED_FORMS_DIRECTORY))));
      else
      {
        ss << '/';
        this->text_parameters.insert(std::pair<Hermes2DApiParam, Parameter<std::string>*>(Hermes::Hermes2D::precalculatedFormsDirPath, new Parameter<std::string>(std::string(ss.str()))));
      }

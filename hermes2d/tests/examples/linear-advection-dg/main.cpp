...
int main(int argc, char* argv[])
{
  ...
  // assemble and solve the finite element problem
	AVector coeff_vec(space.get_num_dofs());
  solve_linear(&space, &wf, matrix_solver, &sln, &coeff_vec);
...
#define ERROR_FAILURE                               -1
	...
	if(std::abs(coeff_vec.get(123)+0.00035659) > eps)
		i++;
	if(std::abs(coeff_vec.get(423)+0.0055674) > eps)
		i++;
	if(std::abs(coeff_vec.get(529) - 0.00283296) > eps)
		i++;
	if(std::abs(coeff_vec.get(997)+0.0188208) > eps)
		i++;
	if(std::abs(coeff_vec.get(1673)+0.0124855) > eps)
		i++;
	if(std::abs(coeff_vec.get(1689) - 0.0981556) > eps)
		i++;
	if(std::abs(coeff_vec.get(1857)+0.12772) > eps)
		i++;

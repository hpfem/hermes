...
extern NumericalFlux _num_flux;
extern double _tau, _rho_ext, _v1_ext, _v2_ext, _p_ext;
extern int _solid_wall_marker, _inlet_outlet_marker;

// Inlet/outlet boundary conditions.
double bc_density(double y) { return _rho_ext; }

// Density * velocity in the x coordinate boundary condition.
double bc_density_vel_x(double y) { return _rho_ext * _v1_ext; }

// Density * velocity in the y coordinate boundary condition.
double bc_density_vel_y(double y) { return _v2_ext; }

// Calculation of the pressure on the boundary.
double bc_pressure(double y) { return _p_ext; }
...
double bc_energy(double y)
{
  ...
  return pressure/(_num_flux.kappa - 1.) + (rho_v_x*rho_v_x+rho_v_y*rho_v_y) / 2*rho;
}
...
{ return (_num_flux.kappa - 1.) * (energy - (rho_v_x*rho_v_x + rho_v_y*rho_v_y) / (2*rho)); }

// Calculates speed of sound.
double calc_sound_speed(double rho, double rho_v_x, double rho_v_y, double energy)
{ return std::sqrt(_num_flux.kappa * calc_pressure(rho, rho_v_x, rho_v_y, energy) / rho); }

// Constant initial state (matching the supersonic inlet state).
scalar ic_density(double x, double y, scalar& dx, scalar& dy) { return _rho_ext; }

scalar ic_density_vel_x(double x, double y, scalar& dx, scalar& dy) { return _rho_ext * _v1_ext; }

scalar ic_density_vel_y(double x, double y, scalar& dx, scalar& dy) { return _rho_ext * _v2_ext; }

scalar ic_energy(double x, double y, scalar& dx, scalar& dy) 
{ return calc_energy(_rho_ext, _rho_ext*_v1_ext, _rho_ext*_v2_ext, _p_ext); }
...
Scalar A_1_0_0(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }

template<typename Scalar>
Scalar A_1_0_1(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 1; }

template<typename Scalar>
Scalar A_1_0_2(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }

template<typename Scalar>
Scalar A_1_0_3(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }

template<typename Scalar>
Scalar A_1_1_0(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{
  return - ((rho_v_x * rho_v_x) / (rho * rho)) + 0.5 * (_num_flux.kappa - 1.) * 
         ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) /   (rho * rho));
}

template<typename Scalar>
Scalar A_1_1_1(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return 2 * (rho_v_x / rho) - (_num_flux.kappa - 1.) * (rho_v_x / rho); }

template<typename Scalar>
Scalar A_1_1_2(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return - (_num_flux.kappa - 1.) * (rho_v_y / rho); }

template<typename Scalar>
Scalar A_1_1_3(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return _num_flux.kappa - 1.; }
...
{
  return - (rho_v_x * energy) / (rho * rho) - (rho_v_x / (rho * rho)) * 
    (_num_flux.kappa - 1.) * (energy - ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) /
    (2 * rho))) + (rho_v_x / rho) * (_num_flux.kappa - 1.) * ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) /
    (2 * rho * rho));
}
...
{
  return (energy / rho) + (1 / rho) * (_num_flux.kappa - 1.) * 
    ( energy - ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) / (2 * rho * rho))) - (_num_flux.kappa - 1.) *
    ((rho_v_x * rho_v_x) / (rho * rho));
}

template<typename Scalar>
Scalar A_1_3_2(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return - (_num_flux.kappa - 1.) * (rho_v_x * rho_v_y) / (rho * rho); }

template<typename Scalar>
Scalar A_1_3_3(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return rho_v_x / rho + (_num_flux.kappa - 1.) * (rho_v_x / rho); }
...
template<typename Scalar>
Scalar A_2_0_0(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }

template<typename Scalar>
Scalar A_2_0_1(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }

template<typename Scalar>
Scalar A_2_0_2(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 1; }

template<typename Scalar>
Scalar A_2_0_3(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }
...
template<typename Scalar>
Scalar A_2_1_3(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy) { return 0; }

template<typename Scalar>
Scalar A_2_2_0(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{
  return - ((rho_v_y * rho_v_y) / (rho * rho)) + 0.5 * (_num_flux.kappa - 1.) *
    ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) /   (rho * rho));
}

template<typename Scalar>
Scalar A_2_2_1(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return - (_num_flux.kappa - 1.) * (rho_v_x / rho); }

template<typename Scalar>
Scalar A_2_2_2(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return 2 * (rho_v_y / rho) - (_num_flux.kappa - 1.) * (rho_v_y / rho); }
...
{
  return - (rho_v_y * energy) / (rho * rho) - (rho_v_y / (rho * rho)) * (_num_flux.kappa - 1.) *
    (energy - ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) / (2 * rho))) + (rho_v_y / rho) *
    (_num_flux.kappa - 1.) * ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) / (2 * rho * rho));
}
...
Scalar A_2_3_2(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{
  return (energy / rho) + (1 / rho) * (_num_flux.kappa - 1.) *
    ( energy - ((rho_v_x * rho_v_x + rho_v_y * rho_v_y) / (2 * rho * rho))) - (_num_flux.kappa - 1.) *
    ((rho_v_y * rho_v_y) / (rho * rho));
}

template<typename Scalar>
Scalar A_2_3_3(Scalar rho, Scalar rho_v_x, Scalar rho_v_y, Scalar energy)
{ return rho_v_y / rho + (_num_flux.kappa - 1.) * (rho_v_y / rho); }
...
double linear_form_interface(int element, int n, double *wt, Func<double> *ue[], Func<double> *v, Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) 
  {
    ...
    result -= wt[i] * v->val[i] * _num_flux.numerical_flux_i(element,w_l,w_r,e->nx[i], e->ny[i]);
  }
  return result * _tau;
}
...
double linear_form_interface_vector(int n, double *wt, Func<double> *ue[], Func<double> *v, Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) 
  {
    ...
    double flux[4];
    _num_flux.numerical_flux(flux,w_l,w_r,e->nx[i], e->ny[i]);

    result[0] -= wt[i] * v->val[i] * flux[0] * _tau;
    result[1] -= wt[i] * v->val[i] * flux[1] * _tau;
    result[2] -= wt[i] * v->val[i] * flux[2] * _tau;
    result[3] -= wt[i] * v->val[i] * flux[3] * _tau;
  }
...
double bdy_flux_solid_wall_comp(int element, int n, double *wt, Func<scalar> *ue[], Func<double> *v, Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) 
  {
    ...
    flux_local[2] = 0;
    flux_local[3] = 0;
    _num_flux.T_rot(mat_rot_inv, -alpha);
    _num_flux.dot_vector(flux, mat_rot_inv, flux_local);
...
double bdy_flux_inlet_outlet_comp(int element, int n, double *wt, Func<scalar> *ue[], Func<double> *v, Geom<double> *e, ExtData<double> *ext)
{
  ...
  for (int i = 0; i < n; i++) 
  {
    ...
    if(e->nx[i] < 0)
    {
      ...
      double sound_speed_1 = sound_speed_l + (_num_flux.R/_num_flux.c_v) * (w_l[1]/w_l[0] - velocity_x_b);
      double rho_1 = std::pow(sound_speed_1*sound_speed_1*w_l[0]/(_num_flux.kappa*calc_pressure(w_l[0], w_l[1], w_l[2], w_l[3])), _num_flux.c_v/_num_flux.R) * w_l[0];
      double velocity_x_1 = velocity_x_b;
      double velocity_y_1 = w_l[2] / w_l[0];

      // Boundary pressure calculated from the intersection state.
      double p_b = rho_1 * sound_speed_1 * sound_speed_1 / _num_flux.kappa;
      ...
      double sound_speed_l_star = _num_flux.R/(_num_flux.c_v * (2+_num_flux.R/_num_flux.c_v)) * w_l[1] / w_l[0] + 2 * sound_speed_l / (2+_num_flux.R/_num_flux.c_v);
      double rho_l_star = std::pow(sound_speed_l_star/sound_speed_l, 2*_num_flux.c_v / _num_flux.R) * w_l[0];
      double velocity_x_l_star = sound_speed_l_star;
      double velocity_y_l_star = w_l[2] / w_l[0];
      double p_l_star = rho_l_star * sound_speed_l_star * sound_speed_l_star / _num_flux.kappa;
      ...
      if(velocity_x_b < sound_speed_1)
      {
        ...
        _num_flux.T_rot(mat_rot_inv, -alpha);
        _num_flux.dot_vector(flux, mat_rot_inv, flux);
      }
    ...
    else
    {
      ...
      double rho_b = w_l[0] * std::pow(p_b/calc_pressure(w_l[0], w_l[1], w_l[2], w_l[3]), (1/_num_flux.kappa));
      double velocity_x_b = (w_l[1] / w_l[0]) + 2*(_num_flux.c_v/_num_flux.R)*(calc_sound_speed<double>(w_l[0], w_l[1], w_l[2], w_l[3]) - std::sqrt(_num_flux.kappa * p_b / rho_b));
      double velocity_y_b = w_l[2] / w_l[0];
      double energy_b = calc_energy<double>(rho_b, velocity_x_b*rho_b, velocity_y_b*rho_b, p_b);

      double sound_speed_l_star = _num_flux.R/(_num_flux.c_v * (2+_num_flux.R/_num_flux.c_v)) * w_l[1] / w_l[0] + 2 * calc_sound_speed<double>(w_l[0], w_l[1], w_l[2], w_l[3]) / (2+_num_flux.R/_num_flux.c_v);
      double rho_l_star = std::pow(sound_speed_l_star/calc_sound_speed<double>(w_l[0], w_l[1], w_l[2], w_l[3]), 2*_num_flux.c_v / _num_flux.R) * w_l[0];
      double velocity_x_l_star = sound_speed_l_star;
      double velocity_y_l_star = w_l[2] / w_l[0];
      double p_l_star = rho_l_star * sound_speed_l_star * sound_speed_l_star / _num_flux.kappa;
      double energy_l_star = calc_energy<double>(rho_l_star, velocity_x_l_star * rho_l_star, velocity_y_l_star * rho_l_star, p_l_star);

...
public:
	CustomWeakFormHeatRK(std::string bdy_air, double alpha, double lambda, double heatcap, double rho,
		double temp_init, double t_final);
...
private:
	...
	public:
		CustomFormResidualSurf(int i, std::string area, double alpha, double rho,
			double heatcap, double temp_init, double t_final)
			: VectorFormSurf<double>(i), alpha(alpha), rho(rho),
			heatcap(heatcap), temp_init(temp_init), t_final(t_final)
		{
			this->set_area(area);
		};

		template<typename Real, typename Scalar>
		Scalar vector_form_surf(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *v, GeomSurf<Real> *e, Func<Scalar> **ext) const;
		...
		// Members.
		double alpha, rho, heatcap, temp_init, t_final;
	};
};